...
function failure(message: string, kind?: string): WireEvent {
  return { type: "failure", message, retryable: TRANSIENT_FAILURE.test(`${kind ?? ""} ${message}`) }
}
...
export function parseSseEvent(raw: unknown): WireEvent | undefined {
  if (!isRecord(raw)) return undefined
  ...
  switch (type) {
    ...
    case "content_block_start": {
      ...
      if (!isJsonObject(raw.content_block)) return failure(`${PROVIDER_NAME} content block was not valid JSON`)
    ...
    case "error": {
      ...
      return failure(message ?? `${PROVIDER_NAME} stream error`, kind)
    }
    default:
      return undefined
  }
...
export function parseToolInput(name: string, partial: string): JsonObject {
  return parseToolArgs(PROVIDER_NAME, name, partial.trim() === "" ? "{}" : partial)
}
